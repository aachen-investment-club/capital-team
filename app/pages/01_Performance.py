...
def _basket_html(df: pd.DataFrame) -> str:
    ...
    for theme in sorted(df["theme"].dropna().unique()):
        ...
        <tr style="background:{BLUE_LIGHT};border-top:2px solid {BORDER};">
          <td colspan="3" style="padding:9px 14px;font-weight:700;color:{NAVY};font-size:13px;">{theme}</td>
        ...
        <tr style="border-bottom:1px solid {BORDER};">
          <td style="padding:6px 14px 6px 30px;color:{TEXT};font-size:12px;">{pos["symbol"]}</td>
          <td style="padding:6px 14px;color:{TEXT_MUTED};font-size:12px;">{pos["name"]}</td>
          <td style="padding:6px 14px;color:{TEXT_MUTED};text-align:center;font-size:12px;">{pos["ccy"]}</td>
          <td style="padding:6px 14px;text-align:right;color:{TEXT};font-size:12px;">{pos["pct_nav"]:.2f}%</td>
          <td style="padding:6px 14px;text-align:right;color:{pos_cr_col};font-size:12px;">{pos_cr:+.2%}</td>
        </tr>"""
...
<tr style="border-bottom:2px solid {BLUE};background:#fff;">
  ...
  <th style="padding:10px 14px;text-align:right;color:{TEXT_MUTED};font-size:11px;font-weight:500;text-transform:uppercase;letter-spacing:.05em;">Cum. Return</th>
</tr>
</thead>
<tbody>
{rows_html}
...
# ── Shared: join weightings with DynamoDB theme mappings ─────────────────────
@st.cache_data
def _weightings_with_themes() -> pd.DataFrame:
    df = get_daily_weightings_history()
    themes = get_theme_mappings()
    return df.merge(themes[["symbol", "theme"]], on="symbol", how="left")
...
with c3:
    selected_bm = st.multiselect("Benchmarks", available_bm, default=["SPY"], key="benchmarks")
...
start_vals = df_perf.groupby("ticker")["index_value"].transform("first")
df_perf["norm_index"] = df_perf["index_value"] / start_vals
df_perf["period_return"] = df_perf["norm_index"] - 1.0

tab_idx, tab_ret = st.tabs(["Index", "Returns"])

with tab_idx:
    fig = px.line(
        df_perf, x="date", y="norm_index", color="ticker",
        labels={"norm_index": "Index (1.0 = period start)", "date": "Date", "ticker": ""},
        template="capital",
    )
...
with tab_ret:
    fig = px.bar(
        df_perf, x="date", y="daily_return", color="ticker",
        barmode="group",
...
c1, c2 = st.columns(2)
with c1:
    pos_start = st.date_input(
        "From", value=_TODAY - timedelta(days=30),
        min_value=_DATE_MIN, max_value=_TODAY, key="pos_start",
    )
with c2:
    pos_end = st.date_input(
        "To", value=_TODAY,
...
if pos_start >= pos_end:
    ...
    with tab_line:
        fig = px.line(
            df_pos, x="date", y="period_return", color="symbol",
            hover_data=["name"],
    ...
    with tab_table:
        ...
        st.dataframe(
            disp.rename(columns={
                ...
                "period_return": "Period Return", "peak": "Max", "trough": "Min",
            }),
...
# ── Holdings detail — flat vs basket view ────────────────────────────────────
st.subheader(f"Holdings — {snap_date.strftime('%d %b %Y')}")

tab_flat, tab_basket = st.tabs(["Flat", "By Basket"])

with tab_flat:
    flat = (
        df_snap[["symbol", "name", "isin", "ccy", "category", "theme", "pct_nav", "cumulative_return"]]
        .copy()
        .sort_values("pct_nav", ascending=False)
    )
    flat["pct_nav"] = flat["pct_nav"].map("{:.2f}%".format)
    flat["cumulative_return"] = flat["cumulative_return"].map("{:+.2%}".format)
    st.dataframe(
        flat.rename(columns={
            "symbol": "Symbol", "name": "Name", "isin": "ISIN", "ccy": "CCY",
            "category": "Asset Class", "theme": "Theme",
            "pct_nav": "% NAV", "cumulative_return": "Cum. Return",
        }),
        use_container_width=True,
        hide_index=True,
    )
...
df_trades = get_performance_trade_log()
periods = sorted(df_trades["date"].dt.to_period("M").unique(), reverse=True)
...
df_month = df_trades[df_trades["date"].dt.to_period("M") == selected_period].copy()
...
if df_month.empty:
    ...
    disp["shares"] = disp["shares"].map("{:,}".format)
    st.dataframe(
        disp.rename(columns={
            "date": "Date", "symbol": "Symbol", "name": "Name",
            "action": "Action", "shares": "Shares", "price": "Price", "value": "Value",
        }),
        use_container_width=True,
        hide_index=True,
        column_order=["Date", "Symbol", "Name", "Action", "Shares", "Price", "Value"],
    )
