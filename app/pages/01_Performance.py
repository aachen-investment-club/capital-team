...
from lib.theme import inject_css, PNG_CONFIG, FAVICON, BENCHMARK_COLORS, DISPLAY_NAMES, _PALETTE, position_color_map

# Light-on-dark colours for the transparent matplotlib export.
# Chosen to be readable on navy (#0C1E40) and distinguishable from each other.
_MPL_COLORS: dict[str, str] = {
...
_DEFAULT_BENCHMARKS = {"60_40", "MSCI_EUROPE", "SPX"}

# Dark → light blue-gray gradient for pie charts (plotly + mpl export)
_PIE_PALETTE = [
...
def _export_returns_matplotlib(df_perf: pd.DataFrame) -> list[str]:
    ...
    def _series_color(ticker: str, i: int) -> str:
        return _MPL_COLORS.get(ticker, _PALETTE[i % len(_PALETTE)])
    ...
    for i, ticker in enumerate(tickers):
        ...
        ax.plot(sub["date"], sub["index_value"],
                label=DISPLAY_NAMES.get(ticker, ticker), color=_series_color(ticker, i),
                linewidth=3, solid_capstyle="round", zorder=2)
    _style(fig, ax)
    ax.set_xlabel("Date", fontsize=F_LABEL, labelpad=10)
    ax.set_ylabel("NAV Multiple  (inception = 1.0)", fontsize=F_LABEL, labelpad=10)
    ax.tick_params(axis="x", rotation=30)
    _legend(ax)
    fig.tight_layout()
    _save(fig, str(downloads / f"returns_index_{ts}.png"))
    ...
    _legend(ax)
    fig.tight_layout()
    _save(fig, str(downloads / f"returns_daily_{ts}.png"))
...
if not bench_ok:
    ...
    with c3:
        selected_bm = st.multiselect(
            ...
            default=[b for b in available_bm if b in _DEFAULT_BENCHMARKS],
            key="benchmarks",
        )
    ...
    if perf_start < perf_end:
        ...
        bm_colors_display = {DISPLAY_NAMES.get(k, k): v for k, v in BENCHMARK_COLORS.items()}
        ...
        clicked = st.button("↓ Export to Downloads", key="export_returns_mpl")
...
if pos_start >= pos_end:
    ...
    # Build colour map from all equity symbols (not just what's in the filtered slice)
    all_equity_symbols = (
        _weightings_with_themes()["symbol"]
        .loc[lambda s: ~s.str.startswith("CASH_")]
        .unique()
        .tolist()
    )
    pos_colors = position_color_map(all_equity_symbols)
...
def _export_pies_matplotlib(df_snap: pd.DataFrame) -> list[str]:
    ...
    def _save_pie(labels, values, suffix):
        ...
        path = str(downloads / f"weights_{suffix}_{ts}.png")
        fig.savefig(path, dpi=200, bbox_inches="tight", transparent=True)
        plt.close(fig)
        saved.append(path)
...
def _pie(df: pd.DataFrame, names: str, values: str) -> object:
    fig = px.pie(df, names=names, values=values, hole=0.35,
                 template="capital", color_discrete_sequence=_PIE_PALETTE)
